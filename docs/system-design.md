...
| **Circuit breaker на внешних API** | Каждая интеграция (YandexGPT, Maps, Booking) защищена circuit breaker с порогами 5 ошибок / 60 секунд. Предотвращает каскадные отказы и экономит ресурсы при массовых сбоях. | Принято |
...
|:---|:---|:---|
| **Web UI** | Streamlit | Приём запросов, отображение маршрута на карте (iframe), кнопки действий, экспорт |
...
| **Planner Agent** | YandexGPT + промпт | Разбивка поездки на дни, логика маршрута, приоритизация активностей |
| **Info/RAG Agent** | YandexGPT + ChromaDB | Поиск информации о достопримечательностях, генерация описаний на основе фактов из RAG |
...
5. [Router → Agents] Делегирование специализированным агентам:
   - plan_trip → Planner → Info/RAG → Booking → Mapper (последовательно)
   - change_plan → Planner (перепланирование дельты)
   - ask_question → Info/RAG (только поиск)
...
3. Planner: извлекает дельту, модифицирует структуру дня 2
4. Info/RAG: поиск альтернатив с фильтром по геолокации
5. Mapper: обновление маршрута на карте
6. Validator: проверка непротиворечивости маршрута
```
...
|:---|:---|:---|
| **YandexGPT API** | 5xx / timeout | Retry 2x → circuit breaker → fallback: сообщение "сервис временно недоступен, попробуйте позже" + частичный ответ из кэша |
| **Maps API** | Timeout / rate limit | Retry 2x → маршрут без карты, текстовое описание пути |
| **Booking APIs** | Timeout / 4xx | Retry 2x → список рекомендуемых отелей из RAG с пометкой "цены могут отличаться" |
